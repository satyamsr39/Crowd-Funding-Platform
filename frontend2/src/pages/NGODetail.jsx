...
export default function NGODetail(){
  ...
  useEffect(()=>{
    fetch((import.meta.env.REACT_APP_BACKEND_URL || 'http://localhost:5000') + '/api/ngos/' + id)
  ...
  async function donate(e){
    e.preventDefault();
    const res = await fetch((import.meta.env.REACT_APP_BACKEND_URL || 'http://localhost:5000') + '/api/donations', {
    ...
    const options = {
      ...
      handler: async (response) => {
        const verify = await fetch((import.meta.env.REACT_APP_BACKEND_URL || 'http://localhost:5000') + '/api/donations/verify', {
          method: 'POST', headers: { 'Content-Type': 'application/json' },
