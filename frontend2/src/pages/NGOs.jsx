      ...
      {ngos.map(n => (
        <div key={n._id} className="bg-white rounded-lg shadow hover:shadow-lg transition p-4">
          <div className="h-40 w-full bg-gray-100 rounded overflow-hidden mb-4">
            <img
              src={n.image ? 
                // If n.image is a full URL, use as is; otherwise prepend backend URL if it's a relative path
                (n.image.startsWith('http') ? n.image : `${backendUrl}/uploads/${n.image}`)
                : 'https://ashaf.org/wp-content/uploads/2020/12/PRYS1937.jpg' // fallback image
              }
              alt={n.name}
